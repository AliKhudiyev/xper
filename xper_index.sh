...
elif [[ $VERSION_SOURCE != $VERSION_TARGET ]]; then
	if ! grep -E "${VERSION_SOURCE}\|.*" -q $INDEX_FP || ! grep -E "${VERSION_TARGET}\|.*" -q $INDEX_FP; then
		echo "[xper_index]: source or target doesn't exist"
